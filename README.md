...
**Published on itch.io after owner approval.** The published source is on `codex/original-c64-presentation`; main still contains the earlier prototype.
...
Gentle pixel smoothing uses a Scale2x-style contour filter. Switch it off for raw pixels. It does not change poses, frame timing, collisions or gameplay. The original animation runs at 50 updates per second. Sound is enabled by default and begins on Play. The splash screen includes a mute control; explicit mute choices are remembered. Sound follows the original SID register/envelope stream; there is no replacement music or ambience loop.
